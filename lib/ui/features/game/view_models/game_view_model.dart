...
class GameViewModelState {
  ...
  bool get isNoMovesLeft {
    if (level == null || isComplete || isTimeOut) return false;
    if (!level!.hasPossibleMoves) return true;

    final history = moveHistory.map((h) => h.tubes).toList();
    if (history.isEmpty) return false;

    final currentTubes = level!.tubes;
    final nextStates = <List<Tube>>[];

    for (int i = 0; i < currentTubes.length; i++) {
      final fromTube = currentTubes[i];
      ...
      for (int j = 0; j < currentTubes.length; j++) {
        if (i == j) continue;
        final toTube = currentTubes[j];
        ...
        nextTubes[i] = fromTube.copyWith(colors: newFromColors);
        nextTubes[j] = toTube.copyWith(colors: newToColors);
        nextStates.add(nextTubes);
      }
    }

    if (nextStates.isEmpty) return true;

    bool tubesListsEqual(List<Tube> a, List<Tube> b) {
    ...
    bool isStateInHistory(List<Tube> state) {
      for (final histState in history) {
        if (tubesListsEqual(state, histState)) {
          return true;
        }
      }
      return false;
    ...
    return nextStates.every((state) => isStateInHistory(state));
  }
